ActiveAdmin.register FoodSpecial do
  index do
  ...
  collection_action :template, :method => :get do
    ...
    send_data(csv_string, :filename => 'Food Specials Import Template.csv', :type => 'text/csv', :disposition => 'attachment')
  end

  action_item do
    link_to "Import", import_admin_food_specials_path
  end

  collection_action :import, :method => :get

  collection_action :upload_import, :method => :post do
    import = CSVImport.new(FoodSpecial, params[:import])
    import.run
    if import.errors.any?
      @errors = import.errors
    else
      flash[:notice] = "Food Specials were imported successfully"
    end
    render :import
  end
end
